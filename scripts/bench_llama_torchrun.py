...
VARIANTS = {
    ...
    "flash": {"flash": True},
    "compile+flash": {"compile": True, "flash": True},
}
...
def _peak_gb(dev) -> float:
    if torch.cuda.is_available() and dev.index < torch.cuda.device_count():
        return torch.cuda.max_memory_allocated(dev) / 1.073742e9
    return 0.0
...
def bench(variant: str, target_gpu_id: int) -> dict:
    ...
    try:
        total_gb = torch.cuda.get_device_properties(dev).total_memory / 1.073742e9
        usable_gb = total_gb - SLACK_GB
        print(
            f"Device Memory: Total={total_gb:.2f} GB, Usable (Est.)={usable_gb:.2f} GB"
        )
    except Exception as e:
        print(f"Warning: Error getting GPU properties: {e}")
    ...
    if use_flash:
        try:
            ...
            cfg_d["_attn_implementation"] = "flash_attention_2"
            print("Using flash_attention_2.")
        except ImportError:
            print(
                "WARNING - flash-attention-2 requested but not found/installed. Falling back."
            )
        if "_attn_implementation" in cfg_d:
            del cfg_d["_attn_implementation"]
    else:
        print("Using default attention implementation.")

    try:
        cfg = LlamaConfig(**cfg_d)
    except Exception as e:
        print(f"FATAL - Failed to create LlamaConfig: {e}")
        return {
            "name": variant,
            "gpu": target_gpu_id,
    ...
    try:
        ...
        if (
            ...
            try:
                model.gradient_checkpointing_enable()
                print("Gradient checkpointing enabled (method found).")
            except Exception as gc_e:
                print(f"WARNING - Failed to enable GC via method: {gc_e}")
        else:
            print("WARNING - Model does not appear to support gradient checkpointing.")
        load_s = time.perf_counter() - t0
        print(f"Model loaded in {load_s:.2f}s.")
    except torch.cuda.OutOfMemoryError:
        print("FATAL - CUDA Out Of Memory during model loading!")
        peak_gb_at_oom = _peak_gb(dev)
        del model
        model = None
        torch.cuda.empty_cache()
        gc.collect()
        return {
            "name": variant,
            "gpu": target_gpu_id,
            "error": "OOM during model load",
            "peak_gb": peak_gb_at_oom,
            "load_s": load_s,
            "usable_gb": usable_gb,
        }
    except Exception as e:
        ...
        if hasattr(torch, "compile"):
            ...
            try:
                ...
                dummy_input = torch.randint(
                    0, cfg.vocab_size, (BATCH, SEQ_LEN), device=dev
                )
                ...
                del dummy_input
                torch.cuda.empty_cache()
                gc.collect()  # Removed loss/backward here
            except torch.cuda.OutOfMemoryError:
                print("ERROR - CUDA Out Of Memory during model compilation/dummy pass!")
                compile_s = time.perf_counter() - t0c
                is_compiled = False
                print("Reloading original model due to compile OOM...")
                del model
                torch.cuda.empty_cache()
                gc.collect()
                try:
                    ...
                    print(f"FATAL - Failed to reload: {reload_e}")
                    return {
                        "name": variant,
                        "gpu": target_gpu_id,
                        "error": "Compile OOM + reload failed",
                        "compile_s": compile_s,
                        "load_s": load_s,
                    }
            except Exception as e:
                ...
                compile_s = 0.0
                is_compiled = False
                print("Continuing benchmark with uncompiled model.")
        else:
        ...
        try:
            ...
            if is_compiled:
                ...
                with torch.amp.autocast(device_type="cuda", dtype=DTYPE):
                    outputs = model(x)
                    loss = outputs.logits.to(torch.float32).mean()
                    loss.backward()  # Full FWD+BWD
            ...
            print("FATAL - OOM during warmup!")
            oom_occurred = True
            error_msg_detail = "OOM during warmup"
        except RuntimeError as e:
            print(f"FATAL - RUNTIME ERROR during warmup: {e}")
            import traceback

            traceback.print_exc()
            oom_occurred = True
            error_msg_detail = f"Runtime Error warmup: {e}"
        except Exception as e:
            print(f"FATAL - UNEXPECTED ERROR during warmup: {e}")
            import traceback

            traceback.print_exc()
            oom_occurred = True
            error_msg_detail = f"Error warmup: {e}"

    if not oom_occurred and model is not None:
        torch.cuda.reset_peak_memory_stats(dev)
    elif model is not None:
        print("Skipping benchmark runs due to failure during warmup.")
    ...
    if not oom_occurred and model is not None:
        ...
        for i in range(REPEATS):
            try:
                ...
                print(f"FATAL - OOM during benchmark iter {i + 1}!")
                oom_occurred = True
                error_msg_detail = f"OOM after {len(times)} reps"
                break
            except RuntimeError as e:
                print(f"FATAL - RUNTIME ERROR during benchmark iter {i + 1}: {e}")
                import traceback

                traceback.print_exc()
                oom_occurred = True
                error_msg_detail = f"Runtime Error after {len(times)} reps: {e}"
                break
            except Exception as e:
                print(f"FATAL - UNEXPECTED ERROR during benchmark iter {i + 1}: {e}")
                import traceback

                traceback.print_exc()
                oom_occurred = True
                error_msg_detail = f"Unexpected Error after {len(times)} reps: {e}"
                break

    # --- Result Calculation & Reporting (Identical to previous version) ---
    peak_gb = _peak_gb(dev)
    n_params = 0
    param_gb = 0
    if model is not None:
        n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        bytes_per_param = 2 if DTYPE in [torch.bfloat16, torch.float16] else 4
        param_gb = n_params * bytes_per_param / 1.073742e9
    else:
        if not error_msg_detail:
            error_msg_detail = "Model loading failed"
    avg_time_s = (sum(times) / len(times)) if times else 0
    tok_s = (BATCH * SEQ_LEN) / avg_time_s if avg_time_s > 0 else 0
    ...
    res = dict(
        ...
        peak_gb=peak_gb,
        param_gb=param_gb,
        act_gb=max(0, peak_gb - param_gb),
        tok_s=tok_s if not final_error_msg else 0,
        t_ms=(1000 * avg_time_s) if not final_error_msg else 0,
        compile_s=compile_s,
        load_s=load_s,
        usable_gb=usable_gb,
        total_gb=total_gb,
        headroom_gb=max(0, usable_gb - peak_gb),
        repeats_completed=len(times),
    )
    if final_error_msg:
        print(
            f"Benchmark FAILED ({final_error_msg}). Final Peak Memory: {peak_gb:.2f} GB."
        )
    else:
        print(
            f"Benchmark done. Final Peak Memory: {peak_gb:.2f} GB. Avg time: {res['t_ms']:.1f} ms. Tok/s: {tok_s:.0f}"
        )
    print("Cleaning up model and cache for this variant...")
    del model
    torch.cuda.empty_cache()
    gc.collect()
    return res
...
def main(args):
    ...
    if target_gpu_id >= torch.cuda.device_count():
        print(
            f"ERROR: GPU ID {target_gpu_id} invalid. Available: {torch.cuda.device_count()}"
        )
        return
    ...
    for variant in ORDER:
        ...
        try:
            result = bench(variant=variant, target_gpu_id=target_gpu_id)
            results.append(result)
        except Exception as e:
            print(
                f"CRITICAL ERROR during execution of bench() for variant {variant}: {e}"
            )
            import traceback
            ...
            results.append(
                {
                    "name": variant,
                    "gpu": target_gpu_id,
                    "error": f"Critical bench error: {e}",
                }
            )
        finally:
            print("--- Cleaning up GPU memory between variants ---")
            torch.cuda.empty_cache()
    ...
    if results:
        ...
        ordered_results_data = [
            results_map.get(v_name) for v_name in ORDER
        ]  # Use ORDER for map lookup too
        for i, r in enumerate(results):  # Iterate through actual results collected
            v_name = r.get(
                "name", ORDER[i] if i < len(ORDER) else "Unknown"
            )  # Handle missing name defensively
            if r is None:
                rows.append(
                    [
                        f"{v_name}?",
                ...
                continue
            status = "OK"
            reps_completed = r.get(
                "repeats_completed", REPEATS if not r.get("error") else 0
            )
            reps_info = f"({reps_completed}/{REPEATS})"
            if r.get("error"):
                status = f"ERROR ({r['error']})"
                reps_info = f"({r.get('repeats_completed', 0)}/{REPEATS})"
            rows.append(
                [
                    r.get("name", "N/A"),
                    r.get("gpu", target_gpu_id),
                    f"{r.get('peak_gb', 0):.1f}" if "peak_gb" in r else "-",
                    f"{r.get('param_gb', 0):.2f}" if "param_gb" in r else "-",
                    f"{r.get('act_gb', 0):.1f}" if "act_gb" in r else "-",
                    f"{r.get('tok_s', 0):.0f}"
                    if "tok_s" in r and status == "OK"
                    else "-",
                    f"{r.get('t_ms', 0):.1f}"
                    if "t_ms" in r and status == "OK"
                    else "-",
                    f"{r.get('compile_s', 0):.1f}"
                    if r.get("compile_s", 0) > 0.01
                    else "-",
                    f"{r.get('load_s', 0):.1f}" if "load_s" in r else "-",
                    f"{r.get('headroom_gb', 0):.1f}" if "headroom_gb" in r else "-",
                    f"{status} {reps_info}",
                ]
            )
        try:
            ...
            print(f"Error generating table: {tab_e}\nRaw results:\n{results}")
        print("-----------------------------------------------------\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Llama benchmarks sequentially on a single GPU."
    )
    parser.add_argument(
        "--gpu", type=int, default=0, help="The ID of the GPU to use (default: 0)."
    )
