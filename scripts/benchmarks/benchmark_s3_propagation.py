...
async def ensure_test_file():
    ...
    if (
        ...
        and os.path.getsize(TEST_FILE_NAME) == TARGET_SIZE
    ):
...
async def benchmark_iteration(comms_instance, iteration):
    ...
    async with comms_instance.session.create_client(
        "s3",
        endpoint_url=comms_instance.get_base_url(
        ...
        while time.time() - poll_start < MAX_POLL_TIME:
            try:
                ...
                print(
                    f"Iteration {iteration}: Object available after {propagation_delay:.2f} seconds."
                )
                break
            except Exception:
    ...
    if propagation_delay is None:
        print(
            f"Iteration {iteration}: Timeout; object not available after {MAX_POLL_TIME} seconds."
        )
