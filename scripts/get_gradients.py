...
def get_grads( uid ):
    ...
    client_config = botocore.config.Config(signature_version="s3v4", max_pool_connections=256)
...
for i, (uid_results, color) in enumerate(zip(all_results, colors)):
    ...
    for result in uid_results:
        ...
        if upload_time >= day_ago:  # Only plot if within last 24 hours
            ax.vlines(x=upload_time, ymin=y_min, ymax=y_max, color=color, alpha=0.3, linewidth=1)
