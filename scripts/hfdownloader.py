    ...
    def wait_all_with_timeout(
        ...
        for future in futures:
            try:
                ...
                print(f"Future timed out after {timeout}s")
                future.cancel()
                results.append(None)
            except Exception as e:
                print(f"Future failed with error: {e}")
                results.append(None)
...
def verify_parquet_file(
    ...
    if actual_size < 8:
        raise Exception(f"File too small to be a valid parquet file (size: {actual_size})")
    
    # Check if expected size matches actual size
    if expected_size != actual_size:
        raise Exception(f"Size mismatch - expected: {expected_size}, actual: {actual_size}")
...
def stream_simple_to_r2(
    ...
    try:
        ...
        if key.endswith(".parquet"):
            ...
            verify_size = actual_content_length if upload_file != local_file else content_length
...
def download_model(
        ...
        try:
            ...
            if r2_key.endswith(".parquet"):
                ...
                try:
                    ...
                    verify_parquet_file(client, r2cfg.bucket_name, r2_key, actual_uploaded_size)
                except Exception as e:
                    print(f"[Worker {worker_id}] Warning: Failed to verify parquet file: {e}")
            ...
            print(f"[Worker {worker_id}] Error processing {file.path}: {e}")
            return False
    ...
    try:
        ...
        worker_manager.wait_all_with_timeout(futures, timeout=7200)

        # Wait for retry queue to empty
        print("Waiting for retry queue to complete...")
        while recovery_manager.has_pending_retries():
            time.sleep(5)
