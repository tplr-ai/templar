...
def main():
    ...
    try:
        ...
        if float(alice_balance) < total_needed:
            print(f"ERROR: Dev account doesn't have enough funds. Balance: {alice_balance}, Needed: {total_needed}")
            sys.exit(1)
        ...
        validator_wallet = bt.wallet(name=config.wallet.name, hotkey=config.validator.hotkey)
