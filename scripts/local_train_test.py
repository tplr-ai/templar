...
class LocalTestMiner(Miner):
    ...
    def __init__(self, config):
        ...
        self.inner_scheduler = torch.optim.lr_scheduler.StepLR(
            self.inner_optimizer, step_size=1, gamma=0.9
        )
        ...
        grad_accum_steps = self.hparams.batch_size // (
            self.hparams.micro_batch_size * self.world_size
        )
    ...
    async def inner_steps(
        ...
        return {
            ...
            "window_entry_loss": total_loss / batch_count if batch_count > 0 else 0,
        }
...
async def main():
    ...
    dummy_input_ids = torch.randint(
        0, vocab_size, (num_dummy_samples, sequence_length), dtype=torch.long
    )
...
def print_io_structure(miner, batch):
    ...
    print(
        f"  - `input_ids` (torch.Tensor): Shape={input_ids.shape}, DType={input_ids.dtype}"
    )
    ...
    print(
        f"  - `outputs.loss` (torch.Tensor): Shape={outputs.loss.shape}, Value={outputs.loss.item():.4f}"
    )
    print(
        f"  - `outputs.logits` (torch.Tensor): Shape={outputs.logits.shape}, DType={outputs.logits.dtype}"
    )
