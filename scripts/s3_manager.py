...
def confirm_action(action_type, scope):
    ...
    """
    while True:
        response = input(f"\nAre you sure you want to {action_type} {scope}? [y/N] ").strip().lower()
        if response == 'y':
            return True
        elif response == 'n':
            return False
        ...
        for page in page_iterator:
            ...
            for obj in contents:
                ...
                if len(keys_to_delete) >= 1000:
                    ...
                    confirm_delete = input(f"You are about to delete {len(keys_to_delete)} files. Proceed? (yes/No): ")
...
def delete_objects_with_prefix(s3_client, bucket_name, prefix):
    ...
    try:
        ...
        if not confirm_action("delete", scope):
            logger.info(f"Deletion of objects with prefix '{prefix}' in bucket '{bucket_name}' cancelled by user.")
            return
...
def delete_objects_with_suffix(s3_client, bucket_name, suffix):
    ...
    try:
        ...
        if not confirm_action("delete", scope):
            logger.info(f"Deletion of objects ending with '{suffix}' in bucket '{bucket_name}' cancelled by user.")
            return
...
def wipe_bucket(s3_client, bucket_name):
    ...
    try:
        ...
        if not confirm_action("wipe", scope):
            logger.info(f"Wiping of bucket '{bucket_name}' cancelled by user at 2nd confirmation.")
            return
...
def main():
    ...
    group_actions.add_argument('--delete-old', type=int, help='Delete objects older than X hours; defaults to 48 if not specified')
    group_actions.add_argument('--prefix', type=str, help='Delete objects with this prefix')
    group_actions.add_argument('--suffix', type=str, help='Delete objects that end with this suffix string')
    group_actions.add_argument('--wipe-bucket', action='store_true', help='Delete all objects in the specified bucket')

    group_config = parser.add_argument_group('Configuration (optional)')
    
    group_config.add_argument('--miner-bucket', action='store_true',
                             help='Use R2_Gradients_* variables to access bucket')
    group_config.add_argument('--dataset-bucket', action='store_true',
                             help='Use R2_Dataset_* variables to access bucket')
    group_config.add_argument('--bucket-name', type=str, 
                             help='Specify a custom bucket name')
    group_config.add_argument('--access-key', type=str,
                             help='Specify an access key instead of environment variables')
    group_config.add_argument('--secret-key', type=str,
                             help='Specify a secret key instead of environment variables')
    group_config.add_argument('--account-id', type=str,
                             help='Specify an account ID instead of environment variables')
    group_config.add_argument('--endpoint-url', type=str,
                             help='Specify an endpoint URL instead of constructing it from the account ID')

    args = parser.parse_args()

    # Check if at least one action is provided
    if (args.delete_old is None and
        args.prefix is None and
        args.suffix is None and
        not args.wipe_bucket):
    ...
    if not args.bucket_name and config:
        ...
        secret_key = args.secret_key or os.getenv('R2_GRADIENTS_WRITE_SECRET_ACCESS_KEY')
