...
import json
from tplr import __version__, logger
...
class DebugManager:
    ...
    async def get_debug_dict(self, window: int) -> Optional[dict]:
        ...
        validator_bucket, validator_uid = await self.comms.chain._get_highest_stake_validator_bucket()
        if not validator_bucket or validator_uid is None:
            self.logger.warning("No validator bucket available for debug data retrieval.")
            return None

        key = f"debug-{window}-{validator_uid}-v{__version__}.pt"
        self.logger.info(f"Attempting to retrieve debug dictionary for window {window} from validator {validator_uid}")

        result = await self.comms.storage.s3_get_object(key=key, bucket=validator_bucket, timeout=20)
        if result is None:
