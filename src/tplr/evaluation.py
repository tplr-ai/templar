...
async def evaluate_peer(uid, state_dict, sync_window, hparams, tokenizer,
    ...
    loader_own, _ = await R2DatasetLoader.get_loader(
        offset=sync_window*hparams.pages_per_window, hparams=hparams, tokenizer=tokenizer,
...
async def evaluate_peers_parallel(evaluation_uids, comms, sync_window, hparams, tokenizer, config, model, transformer, compressor, xshapes, totalks, device, lr, optimizer, scheduler, time_min, time_max):
    """
    ...
    """
    # Load random evaluation data once for all UIDs in this window.
    offset = sync_window * hparams.pages_per_window
    random_loader, random_pages = await R2DatasetLoader.get_loader(
        offset=offset, hparams=hparams, tokenizer=tokenizer,
        data_type="random"
    )
