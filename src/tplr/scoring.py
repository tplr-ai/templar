...
def update_final_score(prev_final_score: float, gradient_score: float, binary_moving_average: float, final_score_ma_alpha: float) -> float:
    """
    ...
    return max(final_score_ma_alpha * prev_final_score + (1 - final_score_ma_alpha) * final_score, 0.0)

def compute_improvement_metrics(loss_before_own: float, loss_after_own: float, loss_before_random: float, loss_after_random: float):
    """
