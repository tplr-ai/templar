        ...
        try:
            if os.path.exists(dashboard_path):
                ...
                for panel in dashboard_json.get('panels', []):
                    if panel.get('id') == 26:  # ID of the version panel
                        content = panel.get('options', {}).get('content', '')
                        
                        # Update the version in the content
                        updated_content = re.sub(
                            r"# Templar Version: v[\d\.]+", 
                            f"# Templar Version: v{version}",
                            content
                        )
                        
                        # Add timestamp to show when it was last updated
                        ...
                        if "Last updated:" in updated_content:
                            ...
                            updated_content = updated_content.replace(
                                "Version data is periodically updated",
                                f"Last updated: {current_time}<br>Version data is periodically updated"
                            )
                ...
                logger.info(f"Updated dashboard version to: v{version} in {dashboard_path}")
                updated = True
