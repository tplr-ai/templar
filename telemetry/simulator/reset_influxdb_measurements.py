...
def reset_measurements(measurement, field=None):
    ...
    try:
        ...
        if not result:
            logger.warning(f"No data found for measurement={measurement}, field={field}")
            return
        
        # Now delete the data
        logger.info(f"Found data to delete. Proceeding with deletion...")
        ...
        delete_api.delete(
            start=start,
            stop=stop,
            predicate=f'_measurement="{measurement}"' + (f' AND _field="{field}"' if field else ""),
            bucket=database,
        )
        
        logger.info(f"Successfully deleted data for measurement={measurement}, field={field}")
...
def main():
    parser = argparse.ArgumentParser(description="Reset InfluxDB measurements to resolve type conflicts")
    parser.add_argument("--measurement", type=str, required=True, help="Measurement to reset")
