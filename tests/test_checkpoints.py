...
from templar.checkpoint import CheckpointManager, upload_checkpoint, get_base_url
from templar import __version__
from templar.config import BUCKET_SECRETS, client_config
...
class TestCheckpointManager(unittest.TestCase):
    ...
    def test_checkpoint_manager_actual_s3_upload(self):
        ...
        self.assertTrue(os.path.exists(checkpoint_manager.checkpoint_path), "Checkpoint file should exist locally")
        ...
        try:
            s3_exists = loop.run_until_complete(
                upload_exists_in_s3(self.bucket_name, filename, self.access_key, self.secret_key, self.endpoint_url)
            )
            self.assertTrue(s3_exists, "Checkpoint file should be uploaded to S3")

            # Clean up: Delete the object from S3
            loop.run_until_complete(
                async_delete_from_s3(self.bucket_name, filename, self.access_key, self.secret_key, self.endpoint_url)
            )
        finally:
            loop.close()
    ...
    def test_checkpoint_upload_failure(self):
        ...
        with mock.patch('templar.checkpoint.BUCKET_SECRETS', {
            ...
            try:
                ...
                self.assertFalse(s3_exists, "Checkpoint file should not be uploaded to S3 due to faulty credentials")
            finally:
                loop.close()
    ...
    def test_checkpoint_cleanup_old_checkpoints(self):
        ...
        self.assertEqual(len(checkpoint_files), 3, "Only the latest 3 checkpoints should be kept locally")
    ...
    def test_checkpoint_save_failure(self):
        ...
        with mock.patch('torch.save', side_effect=IOError("Simulated IO Error")):
            ...
            self.assertFalse(os.path.exists(checkpoint_manager.checkpoint_path), "Checkpoint file should not exist due to save failure")
    ...
    def test_loading_from_highest_stake(self):
        ...
        with mock.patch('templar.checkpoint.download_checkpoint_from_neuron', return_value=self.checkpoint_path):
            # Mock load_checkpoint to simulate loading the checkpoint
            with mock.patch('templar.checkpoint.load_checkpoint', return_value=(100, {})):
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                try:
                    ...
                    self.assertEqual(global_step, 100, "Global step should be loaded from checkpoint")
                finally:
                    loop.close()
