...
async def test_gather_basic_functionality(comms_instance, dummy_compressor):
    ...
    for key in ["0.weightidxs", "0.weightvals"]:
        ...
        assert len(value) == 2, (
            f"Expected 2 tensors for key {key}, got {len(value)}"
        )
...
async def test_missing_vals_key(comms_instance, model, dummy_compressor):
    ...
    responses = [
        ...
        CommsGetResult(data=create_valid_state_dict(model), global_step=20, status="OK"),
        CommsGetResult(data=create_valid_state_dict(model), global_step=30, status="OK"),
    ]
    call_count = 0
            ...
            if response and response.data:
                for key, value in response.data.items():
                    if key.endswith("vals") and value is None:
                        return CommsGetResult(data=None, global_step=None, status="ERROR")
            return response
        return None
...
async def test_weighted_random_sample_no_replacement_zero_weights(comms_instance):
    ...
    assert comms_instance.weighted_random_sample_no_replacement([1, 2, 3], [0, 0, 0], 2) == []


@pytest.mark.asyncio
async def test_weighted_random_sample_no_replacement_k_larger_than_candidates(comms_instance):
    """Test when k is larger than the number of candidates."""
    result = comms_instance.weighted_random_sample_no_replacement([1, 2, 3], [1, 1, 1], 5)
    assert len(result) == 3
...
async def test_weighted_random_sample_no_replacement_basic_selection(comms_instance):
    ...
    result = comms_instance.weighted_random_sample_no_replacement([1, 2, 3, 4], [10, 1, 1, 1], 2)
    assert len(result) == 2
...
async def test_weighted_random_sample_no_replacement_reproducibility(comms_instance):
    ...
    random.seed(42)
    result1 = comms_instance.weighted_random_sample_no_replacement([1, 2, 3, 4], [1, 2, 3, 4], 2)
    random.seed(42)
    result2 = comms_instance.weighted_random_sample_no_replacement([1, 2, 3, 4], [1, 2, 3, 4], 2)
...
async def test_delete_local_directory_non_existent(comms_instance):
    ...
    try:
        ...
        pytest.fail(f"delete_local_directory raised an exception for a non-existent directory: {e}")
...
async def test_cleanup_s3_data(comms_instance):
    ...
    with patch.object(comms_instance, "_get_s3_client") as mock_get_s3_client:
        ...
        deleted_objects = mock_s3_client.delete_objects.call_args[1]["Delete"]["Objects"]
...
async def test_cleanup_old_checkpoints(comms_instance):
    ...
    with patch.object(comms_instance, "_get_s3_client") as mock_get_s3_client:
        ...
        paginator_contents = {
            "Contents": [
                {"Key": "checkpoint-1", "LastModified": datetime.now(timezone.utc) - timedelta(days=3)},
                {"Key": "checkpoint-2", "LastModified": datetime.now(timezone.utc) - timedelta(days=2)},
                {"Key": "checkpoint-3", "LastModified": datetime.now(timezone.utc) - timedelta(days=1)},
...
async def test_post_peer_list(comms_instance):
    ...
    with patch.object(comms_instance, "s3_put_object", new_callable=AsyncMock) as mock_s3_put, patch("os.remove"):
...
async def test_get_bucket_checkpoint(comms_instance):
    ...
    with patch.object(comms_instance, "_get_s3_client") as mock_get_s3_client:
        ...
        checkpoint_data = {"model_state_dict": {"param1": torch.tensor(1)}, "window": 10}
        with patch.object(comms_instance, "s3_get_object", return_value=checkpoint_data):
...
async def test_track_active_peers(comms_instance):
    ...
    with patch.object(comms_instance, "is_miner_active", side_effect=mock_is_miner_active):
...
async def test_load_latest_local_checkpoint(comms_instance):
    ...
    os.makedirs(path1)
    torch.save(checkpoint_data_1, os.path.join(path1, f"checkpoint-1-{comms_instance.uid}-v{version}.pt"))

    path2 = os.path.join(local_dir, "2")
    os.makedirs(path2)
    torch.save(checkpoint_data_2, os.path.join(path2, f"checkpoint-2-{comms_instance.uid}-v{version}.pt"))
...
async def test_get_peer_list(comms_instance):
    ...
    with patch.object(comms_instance, "_get_s3_client") as mock_get_s3_client:
        ...
        with patch.object(comms_instance, "s3_get_object", side_effect=mock_s3_get_object):
...
async def test_s3_get_object_no_load(comms_instance):
    ...
    with patch.object(comms_instance, "_get_s3_client") as mock_get_s3_client:
        ...
        async def mock_get_object(*args, **kwargs):
            return {
                "Body": AsyncMock(
                    **{
                        "__aenter__": AsyncMock(
                            return_value=AsyncMock(read=AsyncMock(return_value=b"test_data"))
                        )
                    }
                )
            }
        ...
        with patch("shutil.move", return_value=key) as mock_move, \
...
async def test_get_latest_checkpoint_local(comms_instance):
    ...
    with patch.object(comms_instance, "_get_highest_stake_validator_bucket", new_callable=AsyncMock) as mock_get_validator_bucket, \
         patch.object(comms_instance, "_get_bucket_checkpoint", new_callable=AsyncMock) as mock_get_bucket_checkpoint, \
         patch.object(comms_instance, "_load_latest_local_checkpoint") as mock_load_local:

        mock_get_validator_bucket.return_value = (None, None)
        mock_get_bucket_checkpoint.return_value = None
        
        local_checkpoint_data = {"model_state_dict": {"param": torch.tensor(1)}, "window": 10}
