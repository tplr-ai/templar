...
def ensure_event_loop():
    ...
    # No yield needed, loop lifetime managed by asyncio based on set_event_loop
...
def wait_for_mock_call(mock_object: Mock, timeout: float = 3.0):
    ...
    while time.monotonic() < start_time + timeout:
        if mock_object.call_count > 0:
            log.debug(f"Mock '{mock_name}' called after {time.monotonic() - start_time:.3f}s. Count: {mock_object.call_count}")
            return True
        time.sleep(0.05)
    log.warning(f"Timeout ({timeout}s) waiting for mock '{mock_name}'. Call count remained {mock_object.call_count}.")
    return False
...
class TestMetricsLogger:
    ...
    def mock_influxdb_client(self):
        ...
        with patch("tplr.metrics.InfluxDBClient", autospec=True) as mock_client_class:
            ...
            mock_client_instance.ping.return_value = True # Mock ping if called during init
            mock_write_api_instance = MagicMock(spec_set=["write"]) # Use spec_set if methods are known
            mock_write_api_instance.write._mock_name = "mock_write_api_instance.write" # For logs
    ...
    @pytest.fixture
    def metrics_logger(self, mock_influxdb_client): # Depends on the mock client fixture
        ...
        logger = MetricsLogger(
            prefix="test", uid="123", role="miner", group="test_group", job_type="training",
            host="dummyhost.invalid", port=8086, token="unit-test-token",
            org="unit-test-org", database="unit-test-bucket"
        )
        log.debug(f"MetricsLogger instance created [ID: {id(logger)}]")
        # Verify internal mock references immediately
        assert logger.client is mock_influxdb_client.return_value, "Logger client mismatch"
        assert logger.write_api is mock_influxdb_client.return_value.write_api.return_value, "Logger write_api mismatch"
        assert isinstance(logger.write_api.write, MagicMock), "Logger write_api.write is not a mock"
        return logger
    ...
    def mock_cuda_functions(self):
        try:
            ...
            with mock.patch.multiple("tplr.metrics.torch.cuda", **{
                ...
                "current_device": mock.DEFAULT, "is_available": mock.DEFAULT,
            }) as mocks:
    ...
    def bt_config(self):
        ...
        config.logging = MagicMock(); config.logging.debug = False; config.logging.trace = False
        return config
    ...
    def test_init(self, mock_influxdb_client):
        ...
        dummy_host="dummyhost.invalid"; dummy_port=8086; test_token="init-test-token"; test_org="unit-test-org"
        _ = MetricsLogger(token=test_token, host=dummy_host, port=dummy_port, org=test_org)
        mock_influxdb_client.assert_called_once()
        _, call_kwargs = mock_influxdb_client.call_args
        expected_url = f"https://{dummy_host}:{dummy_port}"; expected_org = test_org
    ...
    def test_process_value(self, metrics_logger):
        ...
        assert metrics_logger.process_value(42) == 42; assert metrics_logger.process_value(3.14) == 3.14
        peer_ids = [1, 2, 3]; assert metrics_logger.process_value(peer_ids) == str(peer_ids)
        values = [1.1, 2.2, 3.3]; expected_stats = {'mean': 2.2, 'min': 1.1, 'max': 3.3, 'median': 2.2}
        assert metrics_logger.process_value(values) == pytest.approx(expected_stats)
        assert metrics_logger.process_value([]) == 0.0; assert metrics_logger.process_value("test") == "test"; assert metrics_logger.process_value(None) == 0.0
    ...
    def test_log_basic(self, metrics_logger):
        ...
        assert wait_for_mock_call(write_mock), f"'{write_mock._mock_name}' not called within timeout"
    ...
    def test_log_with_exception(self, metrics_logger):
        ...
        assert wait_for_mock_call(write_mock), f"'{write_mock._mock_name}' not called within timeout (even with exception)"
    ...
    def test_miner_metrics_pattern(self, metrics_logger, mock_system_metrics, mock_cuda_functions):
        ...
        metrics_logger.log("training_step", {"w": 42}, {"l": 0.75}, with_system_metrics=True, with_gpu_metrics=True)
        write_mock = self.get_write_method_mock(metrics_logger)
        assert wait_for_mock_call(write_mock), f"'{write_mock._mock_name}' not called within timeout"
        write_mock.assert_called_once()
