...
from tplr.wandb import initialize_wandb


class TestWandb(unittest.TestCase):
    @patch("tplr.wandb.wandb")
    @patch("tplr.wandb.os.path.exists", return_value=False)
    @patch("tplr.wandb.open", new_callable=unittest.mock.mock_open)
